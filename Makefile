...
test:
	...
	$(CXX) -O3 -Wall -Werror -Wextra -pedantic -g -std=c++11 $(shell python3-config --cflags) -o Gomoku $(filter-out bindings.cpp, $(wildcard *.cpp)) $(shell python3-config --ldflags) # -pg -> profiling
#	$(CXX) -O3 -fsanitize=address,leak,undefined -Wall -Werror -Wextra -pedantic -std=c++11 $(shell python3-config --cflags) -o Gomoku $(filter-out bindings.cpp, $(wildcard *.cpp)) $(shell python3-config --ldflags)
